...
public:
	...
	// 「使用可」「使用不可」を設定する
	void Alive(bool alive) { this->alive = alive; }

	// 位置を取得する Get position
	DirectX::SimpleMath::Vector2 Position() { return position; }
	// 位置を設定する Set position
	void Position(DirectX::SimpleMath::Vector2 position) { this->position = position; }

	// 弾回転角を取得する Get angle
	float Angle() { return this->angle; }
	// 弾回転角を設定する Set angle
	void Angle(float angle) { this->angle = angle; }

	// 色を取得する Get color
	DirectX::SimpleMath::Vector4 Color() { return this->color; }
	// 色を設定する Set color
	void Color(DirectX::SimpleMath::Vector4 color) { this->color = color; }
